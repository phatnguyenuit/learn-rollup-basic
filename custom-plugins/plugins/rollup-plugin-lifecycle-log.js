...
const hookLog = (hookName) => `Hook ${cyanBright(`'${hookName}':`)}`;
const paramLog = (paramName, paramValue) => {
    const stringified = JSON.stringify(paramValue, null, 2);
    const text = (stringified === null || stringified === void 0 ? void 0 : stringified.length) > maxLineLength
...
const plugin = () => ({
    ...
    resolveId: (source, importer) => {
        log(hookLog("resolveId"), paramLog("source", source), paramLog("importer", importer));
        return null;
    },
    load: (id) => {
    ...
    resolveDynamicImport: (specifier, importer) => {
        log(hookLog("resolveDynamicImport"), paramLog("specifier", specifier), paramLog("importer", importer));
        return null;
    },
    buildEnd: (error) => {
    ...
    renderStart: (outputOptions, inputOptions) => {
        log(hookLog("renderStart"), paramLog("outputOptions", outputOptions), paramLog("inputOptions", inputOptions));
    },
    banner: () => {
        log(hookLog("banner"));
        return "";
    ...
    renderChunk: (code, chunk, options) => {
        log(hookLog("renderChunk"), paramLog("code", code), paramLog("chunk", chunk), paramLog("options", options));
        return null;
    },
    renderError: (error) => {
    ...
    generateBundle: (options, bundle, isWrite) => {
        log(hookLog("generateBundle"), paramLog("options", options), paramLog("bundle", bundle), paramLog("isWrite", isWrite));
    },
    writeBundle: (options, bundle) => {
        log(hookLog("writeBundle"), paramLog("options", options), paramLog("bundle", bundle));
    },
    /**
     * Watch Hooks
     */
